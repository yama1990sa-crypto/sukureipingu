...
Indeed以外の汎用モードでは、まず一覧ページからタイトル・URLを検出し、
続けて各項目の詳細ページを実際に開いて company・location(住所)・
phone・email をページ本文から正規表現で自動検出します(株式会社等の
法人格、郵便番号・都道府県名、電話番号形式、メールアドレス形式で判定)。
詳細ページを開く件数は「取得ページ数」欄で指定でき、最大30件までです
(サイトへの負荷とサーバーの実行時間を考慮した上限です)。
それでも本文に情報が書かれていないサイトでは検出できず空欄になります。
