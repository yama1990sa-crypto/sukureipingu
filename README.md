...
# 企業名から公式サイト・問い合わせ先を検索(カンマ区切り)
python indeed_scraper.py --companies "A株式会社,B株式会社" --pages 10 --output companies.csv
...
```
※ `--pages` は詳細ページを開く件数、または企業名検索モードでは検索する社数の
上限(最大30)として使われます。
...
### 企業名から探すモード
company(入力した企業名) / official_url(公式サイトと思われるURL) /
contact_url(お問い合わせページのURL) / phone(電話番号) / email(メールアドレス) /
address(住所) / note(見つからなかった場合の理由など)

入力した企業名ごとに検索エンジン(DuckDuckGo)で「企業名 公式サイト」を検索し、
最初に見つかった候補を公式サイトとみなします。そのページを開いて本文から
電話番号・メールアドレス・住所を抽出し、あわせて「お問い合わせ」「Contact」等の
リンクを探して開き、より詳しい情報があれば上書きします。

**注意:** 検索結果は必ずしも正しい公式サイトとは限りません(同姓同名の別会社、
関連会社のページ、検索結果の精度などにより誤って別サイトの情報を取得する
可能性があります)。取得した内容は必ず目視で確認してから利用してください。
...
- ブロック検知は簡易的なキーワード判定です。過信せず、実行結果(CSVの件数)を
  都度確認してください。
- URLから調べるモードの会社名・住所・電話番号・メールアドレス抽出は
  正規表現によるヒューリスティックです。サイトの書き方によっては
  誤検出・未検出があります。
- 企業名から探すモードは検索エンジンの結果に依存するため、社名が一般的な
  単語と紛らわしい場合や同名の会社が複数ある場合は誤ったサイトを
  拾ってしまうことがあります。
